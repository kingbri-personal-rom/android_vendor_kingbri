...
func init() {
	android.RegisterModuleType("kingbri_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = kingbriExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = kingbriExpandVariables(ctx, dep_file)
	...
	cmd := kingbriExpandVariables(ctx, String(g.properties.Cmd))
