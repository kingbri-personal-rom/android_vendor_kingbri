...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$GAHS_ROOT"/prebuilts/tools-gahs/common/smali/baksmali.jar
        export SMALIJAR="$GAHS_ROOT"/prebuilts/tools-gahs/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$GAHS_ROOT"/prebuilts/tools-gahs/"${HOST,,}"-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$GAHS_ROOT"/prebuilts/tools-gahs/"${HOST,,}"-x86/bin/compact_dex_converter
    fi
